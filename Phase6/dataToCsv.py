...
def dbToCsv():
	...
	cursor = db.course.aggregate([
		{
			'$match':{'$or':[{'code':{'$regex':'^COMP4332'}},{'code':'COMP1942'},{'code':{'$regex':'^RMBI4310'}}]}
		},
		{
			'$unwind': '$sections'
		},
	...
	with open(fileName,'w', encoding='utf-8', newline='') as outfile:
		...
		for answers_record in cursor:  # Here we are using 'cursor' as an iterator
			...
			if("COMP1942" in answers_record_id or "COMP4332" in answers_record_id or "RMBI4310" in answers_record_id):
			# if("COMP1942" in answers_record_id):
				...
				if(re.match(r'L\d',answer_record_sectionId)):
					flattened_record = {
						...
						'recordTime': (answer_record['recordTime'].timestamp() - earliestTime)/1800,
	...
	for index in range(len(fullContinueData[:-1])+missingCount):
		if int(fullContinueData[index]['recordTime']) + 1 < int(fullContinueData[index+1]['recordTime']):
			fullContinueData.insert(index+1, {'code':fullContinueData[index]['code'] , 'sectionId': fullContinueData[index]['sectionId'], 'recordTime': float(fullContinueData[index]['recordTime'])+1, 'quota': -1 , 'enrol': -1 , 'wait': -1, 'total':-1})
			missingCount += 1
