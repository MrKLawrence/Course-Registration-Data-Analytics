...
# def generateNewData(newTimeSeriesNoOutputDataFilename):
# 	# to read the data from the file
...
# 	newDataX, newDataY = create_dataset(data_float_TwoDim)
...
def trainModel(trainDataX, trainDataY):
	...
	model = Sequential()
	#model.add(GRU(1, input_shape=(look_back,4)))
	...
	model.fit(trainDataX, trainDataY, validation_split=0.2, epochs=20, batch_size=2)
...
def deNormalize(testDataX, newY_Predict, prefix=""):

	# to re-map the normalized values to the original values (New 1)
	scaler_filename = str(prefix)+"scaler.save"
	scaler = joblib.load(scaler_filename) 
	# print("testDataX.shape, newY_Predict.shape")
	# print(testDataX.shape, newY_Predict.shape)
	
	# data_float_TwoDim (New 1)
	# data_float_TwoDim = scaler.inverse_transform(data_float_TwoDim)
	# newY_Predict (New 1)
	# newY_Predict = numpy.insert(newY_Predict,1, [[0],[0],[0]], axis=1)
	# newY_Predict = newY_Predict.reshape(len(newY_Predict), 1)
	allData = numpy.hstack((testDataX,newY_Predict))
	newY_Predict = scaler.inverse_transform(allData)
	# print("Denormal newY_Predict:")
	# print(newY_Predict)
	testDataX = newY_Predict[:,:3]
	newY_Predict = newY_Predict[:,3:]

	# print("testDataX.shape, newY_Predict.shape")
	# print(testDataX.shape, newY_Predict.shape)
	# newY_Predict = newY_Predict.reshape(len(newY_Predict))
	# trainDataX (New 1)
	# trainDataX = trainDataX.reshape(len(trainDataX), 1)
	# trainDataX = scaler.inverse_transform(trainDataX)
	# trainDataX = trainDataX.reshape(len(trainDataX))
	# # trainDataY (New 1)
...
def predictionHandler(courseCode,lectureNumber, timeslot):
	...
	numpyX = df_row.values[0][2:6]
	# print("numpyX1")
	# print(numpyX)
	numpyX = numpy.reshape(numpyX,(1,len(numpyX)))
	# print("numpyX2")
	# print(numpyX)
	scaler_filename = str(courseCode)+"scaler.save"
	scaler = joblib.load(scaler_filename)
	numpyX = scaler.transform(numpyX)
	# print("numpyX3")
	# print(numpyX)
	numpyX = numpyX[:,:3]
	# print("numpyX4")
	...
	modelFilenamePrefix = str(courseCode)+"-NN-Model"
...
def main():
	...
	for prefix in coursesListForPredict:
		...
		newTargetAttributeDataFilename = prefix+"New-NN-Output.csv"
		modelFilenamePrefix = prefix+"-NN-model"#newly added

		# look_back = 0  #newly added
		...
		#numpyX, numpyY = getXY(trainingDataFile)
...
x=predictionHandler("COMP1942",1,59)
print(x)
# 287,135,135
