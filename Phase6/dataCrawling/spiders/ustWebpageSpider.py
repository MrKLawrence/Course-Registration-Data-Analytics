...
class ustWebpageSpider(scrapy.Spider):
	...
	def parse_snapshot(self, response):
		for a in response.xpath('//div[@class="depts"]/a'):
			...
			# if("RMBI" in str(a)):
			#only craw data from COMP course
			# if("COMP" in str(a)):
	...
	def parse_course(self, el, semester, record_time):
		...
		# if("COMP1942" not in code and "COMP42" not in code and "COMP43" not in code and "RMBI" not in code):
		# 	return
	...
	def parse_sections(self, trs, code, semester, record_time):
		...
		for tr in trs:
			...
			if 'newsect' in class_name:
				...
				offering = {
					# index starts from 1
					# TODO: split dateAndTime to daysOfWeek and time - to be finished in dataToCsv.py
