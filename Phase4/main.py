...
#  The program is written for the phase 3
#
...
import subprocess	#phase 3
import scrapy		#phase 3
import time			#phase 3
import random		#phase 3
...
class OneWebpageSpider(scrapy.Spider):
	...
	def parse(self, response):
		...
		#Question: Is it here to set waiting time?
		print("Sleeping for 5.1 seconds...")
		time.sleep(5.1)

		print("")

		print("Sleeping for some seconds (from 1 second to 70 seconds)...")
		duration = random.uniform(1, 70)
		print("   in this case,", duration, "seconds....")
		time.sleep(duration)
...
def dataCrawling(db,url):
	if(str(url).lower() == "default" or url == ""):
		...
		strCommand = "scrapy crawl CourseRegistrationDataAnalytics"
		subprocess.run(strCommand, shell=True)
	else:
